...
# Generate marks:
# '@param Q, number of expression level, 2 in our analysis
# '@param Theta, interaction matrix between highly expressed and lowly expressed
# '@param lambda, decay parameter
# '@param c, cutoff of interaction distance
# '@param omega, abundance of highly and lowly expressed 
...
z_generator = function(x, y, Q, Theta, lambda, c, omega, iter, seed) {
  ...
  z_s <- sample(0:(Q-1), n, replace = TRUE, prob = exp(omega)/sum(exp(omega)));
  print(sum(z_s));
  z <- z_s;
